...
/**
 * Read all data from a readable stream into a string.
 *
 * @param ReadableStream $source The stream to buffer.
 * @param Cancellation|null $cancellation Optional cancellation.
 * @param int $limit Maximum number of bytes to read.
 *
 * @return string Buffered contents of the stream.
 */
function buffer(ReadableStream $source, ?Cancellation $cancellation = null, int $limit = \PHP_INT_MAX): string
{
    ...
    while (null !== $chunk = $source->read($cancellation)) {
        ...
        if ($length > $limit) {
            throw new StreamException("Buffered stream exceeded the limit of {$limit} bytes");
        }
    }
