...
use Fledge\Async\File;
use Fledge\Async\ForbidCloning;
use Fledge\Async\ForbidSerialization;
use Fledge\Async\Future;
use Fledge\Async\Database\Mysql\MysqlColumnDefinition;
...
use Fledge\Async\Database\SqlTransientResource;
...
class ConnectionProcessor implements SqlTransientResource
{
    ...
    public function connect(?Cancellation $cancellation = null): void
    {
        ...
        // if a charset is specified, we need to set it before any query, since the
        // handshake charset only covers the connection, not the session defaults
    ...
    private function handleLocalInfileRequest(string $packet): void
    {
        EventLoop::queue(function () use ($packet): void {
            try {
                ...
                /** @var \Fledge\Async\File\File $fileHandle */
                if (!\function_exists("Fledge\Async\\File\\openFile")) {
                    throw new \Error("fledge-fiber/file must be installed for LOCAL INFILE queries");
                }
