...
use Fledge\Async\Stream\SocketAddressType;
...
final class SocketIpcHub implements IpcHub
{
    ...
    public function __construct(
        private readonly Stream\ServerSocket $server,
        ...
        $this->accept = static function () use (
            ...
            while ($waitingByKey) {
                ...
                if (!$client) {
                    $queued = false;
                    $exception = new Stream\SocketException('IPC socket closed before the client connected');
    ...
    public function accept(string $key, ?Cancellation $cancellation = null): ResourceSocket
    {
        if ($this->server->isClosed()) {
            throw new Stream\SocketException('The IPC server has been closed');
        }
